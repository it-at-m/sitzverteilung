...
export function mapCalculationResultToResultData(
    ...
    for (const groupName in calculationResult.proportions) {
        ...
        const resultData: ResultData = {
            ...
            staleResults: { // Neues Feld für das stale-Objekt
                [CalculationMethod.D_HONDT]: { groupNames: [], amountSeats: 0, ratio: 0 },
                [CalculationMethod.HARE_NIEMEYER]: { groupNames: [], amountSeats: 0, ratio: 0 },
                [CalculationMethod.SAINTE_LAGUE_SCHEPERS]: { groupNames: [], amountSeats: 0, ratio: 0 },
            },
        };

        for (const method of Object.keys(calculationResult.methods) as CalculationMethod[]) {
                ...
                if (method === CalculationMethod.D_HONDT) {
                    ...
                    if (validation && validation[groupName]) {
                        ...
                        resultData["D'Hondt-validation"] =
                            !validationData.overRounding && !validationData.lostSafeSeat ? 1 : 0; // Anpassung der Validierung
                    }
                    ...
                    resultData["Sainte-Laguë/Schepers-seats"] = distribution[groupName] || 0;
